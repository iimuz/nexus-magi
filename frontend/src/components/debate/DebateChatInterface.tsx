...
import { Box, Container, TextField, Button, Typography, Paper, Slider } from '@mui/material';
...
const DebateChatInterface: React.FC = () => {
  ...
  const [messages, setMessages] = useState<Message[]>([
    {
      role: 'assistant',
      content: 'こんにちは！MAGI合議システムです。どのようなご質問がありますか？',
    },
  ]);
      ...
      onMelchiorResponse: (response: string, phase?: string) => {
        ...
        setMagiResponse((prev) => {
          const updated = {
            ...prev,
            melchior: response,
            phase: (phase as 'initial' | 'thinking' | 'debating' | 'final') || prev.phase,
          };
      ...
      onConsensusResponse: (response: string, phase?: string) => {
        ...
        setMagiResponse((prev) => {
          const updated = {
            ...prev,
            consensus: response,
            phase: (phase as 'initial' | 'thinking' | 'debating' | 'final') || 'final',
          };
